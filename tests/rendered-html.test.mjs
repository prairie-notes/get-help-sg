...
test("contains the guided flow, safety controls, clinic tools and no starter-only dependencies", async () => {
  ...
  assert.doesNotMatch(page, /Little Cross Family Clinic/);
  assert.doesNotMatch(page, /Uniq Medical Clinic \(Yishun\)/);
  assert.match(page, /1 North Bridge Road, High Street Centre #06-11, Singapore 179094/);
  assert.match(page, /href: "https:\/\/www\.mindcarespecialists\.com\/"/);
  assert.match(page, /fees: "Not published online - confirm the current consultation and follow-up fees\."/);
  assert.match(page, /booking: "Book an appointment via their website form"/);
  assert.match(page, /source: "Mind Care Clinic contact page"/);
  assert.match(page, /bookingHref: "https:\/\/www\.mindcarespecialists\.com\/make-an-appointment\/"/);
  assert.match(page, /sourceHref: "https:\/\/www\.mindcarespecialists\.com\/make-an-appointment\/"/);
  assert.match(revisedHome, /clinic\.sourceHref \?\? clinic\.href/);
  assert.match(page, /kind: "psychiatrist",[\s\S]{0,240}practice: "Private Space Medical",[\s\S]{0,180}address: "541 Orchard Road, #19-01 Liat Towers, Singapore 238881"/);
  assert.match(page, /kind: "therapist",[\s\S]{0,240}practice: "Private Space Medical",[\s\S]{0,180}address: "1 Farrer Park Station Road, #11-09 Farrer Park Medical Centre, Singapore 217562"/);
  assert.match(page, /kind: "therapist",[\s\S]{0,240}practice: "Private Space Medical",[\s\S]{0,180}address: "541 Orchard Road, #19-01 Liat Towers, Singapore 238881"/);
  assert.doesNotMatch(page, /1 Farrer Park Station Road, #11-16\/17\/18 Farrer Park Medical Centre, Singapore 217562/);
  assert.match(page, /The Psychiatric and Behavioural Medicine Clinic\|3 Mount Elizabeth, #15-08 Mount Elizabeth Medical Centre, Singapore 228510/);
